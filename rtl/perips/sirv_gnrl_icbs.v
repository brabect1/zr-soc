...
//  wire outs_cnt_r;
  ...
  always @(posedge clk or negedge rst_n) begin: p_outs_cnt
    if (!rst_n)
      outs_cnt_r <= {OUTS_CNT_W{1'b0}};
    else if (outs_cnt_ena)
      outs_cnt_r <= outs_cnt_nxt;
  end: p_outs_cnt
...
//  wire apb_enable_r;
  ...
  always @(posedge clk or negedge rst_n) begin: p_apb_enable
    if (!rst_n)
      apb_enable_r <= 1'b0;
    else if (apb_enable_ena)
      apb_enable_r <= apb_enable_nxt;
  end: p_apb_enable
